...
import com.schwabapi.marketdata.deserializers.MarketsHashMap;
import com.schwabapi.marketdata.deserializers.QuoteResponseHashMap;
...
import com.schwabapi.marketdata.model.pricehistory.Candle;
...
public class SchwabMarketDataApiClient {
    ...
    @Autowired
    @Qualifier("schwabMarketDataWebClient")
    ...
    public List<Hours> fetchMarkets(@NotNull List<Market> markets,
        ...
        if (markets.size() > 0) {
            ...
            if (marketsMap != null && marketsMap.map != null && marketsMap.map.size() > 0) {
                ...
                throw new SymbolNotFoundException("Market Hours for '" + markets + "' not found");
            }
        } else {
